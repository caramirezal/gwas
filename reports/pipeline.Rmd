...
### **14. Heterozygosity vs missing SNPs**

**INPUT**: data/qc_miADN_plus_mxn/miADN_plus_mxn_updated_sex_filter_missnp

Perform heterozygosity score calculation.

```
./inst/plink 
     --bfile data/qc_miADN_plus_mxn/miADN_plus_mxn_updated_sex_filter_missnps \
     --missing \
     --out data/qc_miADN_plus_mxn/miADN_plus_mxn_updated_sex_filter_missnps
```
...
* miADN_plus_mxn_updated_sex_filter_missnps.imiss - missing genotypes ratio. 
* miADN_plus_mxn_updated_sex_filter_missnps.het - heterozigosity in each sample.
...
het <- read.table("../data/qc_miADN_plus_mxn/miADN_plus_mxn_updated_sex_filter_missnps.het",
                  fill=TRUE, sep="", header = TRUE)

## Read snp missing data
missing <- read.table("../data/qc_miADN_plus_mxn/miADN_plus_mxn_updated_sex_filter_missnps.imiss",
                      fill=TRUE, sep="", header = TRUE)
...
ggplot(data=check, aes(x=F_MISS, y=HET, colour=ETNIA)) + 
        ...
        geom_hline(yintercept = 0.3015, color="red", 
                   linetype="dotted", size=0.8) +
...
### **11. Perform a sex check** 

**INPUT**: miADN_plus_mxn_updated_sex (bed/bim/fam)
...
There were 146 sex discrepancies. Create a list of samples that do not pass the sex check. 

```
grep PROBLEM data/qc_miADN_plus_mxn/miADN_plus_mxn.sexcheck | cut -f1,2 > data/qc_miADN_plus_mxn/fail-sex-check.txt
```

**OUTPUT**: 

* miADN_plus_mxn.sexcheck - contains a report with sex discrepancies as inferred from homozygocity scores. 
...
* miADN_plus_mxn_updated_sex (bed/bim/fam) files.

```
## missing data check
./inst/plink 
    --bfile data/genotyping/qc/miADN_updated_sex 
    --missing
    --out data/genotyping/qc/miADN
```

**OUTPUT**: miADN_plus_mxn.imiss

### **13. Calculation of missing SNP information**
...
**OUTPUT**: miADN_plus_mxn.het.

The following plot shows missing SNP information.

![Missing SNPs frequency](../figures/missing_info_SNPs.jpg)

It can be seen that there is a group of SNPs poorly genotyped above 0.5. Hence, SNPs were filtered using this threshold. Filtering poorly genotyped at this point is important to avoid removing to many samples through the QC.

```
## Remove poorly genotyped markers
./inst/plink 
    --bfile data/qc_miADN_plus_mxn/miADN_plus_mxn_updated_sex 
    --maf 0.01 
    --geno 0.5 
...
NOTE: Re-run --missing parameter as shown above over the miADN_plus_mxn_updated_sex_filter_missnps output file. Then, plot the Missing SNPs histogram to verify that SNPs above 0.5 were filtered out.




### **15. Identity by state calculation**

**INPUT**: miADN_plus_mxn_updated_sex_filter_missnps.
...
./inst/plink \
    --bfile data/qc_miADN_plus_mxn/miADN_plus_mxn_updated_sex_filter_missnps \
    --extract data/qc_miADN_plus_mxn/miADN_plus_mxn.prune.in \
    --genome \
    --out data/qc_miADN_plus_mxn/miADN_plus_mxn 
```